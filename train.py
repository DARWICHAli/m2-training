...
def evaluate_metrics(model, dataloader, text_field, test=False):
    ...
    gts = evaluation.PTBTokenizer.tokenize(gts, add_eos=False)  # 这里做没啥问题，因为多轮Tokenize在验证/测试集上没影响
    gen = evaluation.PTBTokenizer.tokenize(gen, add_eos=False)  #
    print('examples:')
    print('gen:', gen['0_0'])
    print('gt:', gts['0_0'])
...
def encode_caps_gt(caps_gts, eos_idx):
    encoded_caps_gt = {}
...
def train_scst(model, dataloader, optim, cider, text_field, test=False, spice=None):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, caps_gt) in enumerate(dataloader):
            ...
            my_caps_gt = encode_caps_gt(replicated_caps_gt, text_field.vocab.stoi['<eos>'])
            reward = cider.compute_score(my_caps_gt, my_caps_gen)[1].astype(np.float32)
            if spice is not None:
                caps_gen = text_field.decode(outs.view(-1, seq_len))
                caps_gt = list(itertools.chain(*([c, ] * beam_size for c in caps_gt)))
                caps_gen, caps_gt = tokenizer_pool.map(evaluation.PTBTokenizer.tokenize, [caps_gen, caps_gt])
                spice_score = np.array(spice.compute_score(caps_gt, caps_gen)[1]).astype(np.float32)
                reward = 0.5 * (reward + spice_score)
...
def encode_corpus(corpus, stoi, eos_idx):
    encoded_corpus = {}
    fix_stoi = {}
    ...
    for key, value in tqdm(corpus.items(), desc='encode corpus'):
        ...
        encoded_corpus[key] = [encoded_words + [eos_idx]]
...
def build_cider_train(ref_caps_train):
    ...
    encoded_corpus = encode_corpus(corpus, text_field.vocab.stoi, text_field.vocab.stoi['<eos>'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_m', type=int, default=0)
    ...
    parser.add_argument('--spider', action='store_true', default=False)
    ...
    # Create the dataset
    dataset = COCO(image_field, text_field, 'coco/images/', args.annotation_folder)
    print('dataset time')
    print(time.time() - start)
    start = time.time()
    ...
    spice_train = Spice() if args.spider else None
    cider_train = None
    ...
    if args.test:
        ...
        metrics = evaluate_metrics(model, dict_dataloader_test, text_field, test=True)
        print(metrics)
        ...
        train_scst(model, dict_dataloader_train, optim, cider_train, text_field, test=True, spice=spice_train)
        print('test done')
        exit(0)
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if not use_rl:
            ...
            if cider_train is None:
                cider_train = build_cider_train(ref_caps_train)
            train_loss, reward, reward_baseline = train_scst(model, dict_dataloader_train, optim, cider_train, text_field, spice=spice_train)
            writer.add_scalar('data/train_loss', train_loss, e)
