...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        roots = {}
        roots['train'] = {
            'img': os.path.join(img_root, 'train2017'),
            'cap': os.path.join(ann_root, 'captions_train2017_tokenized.json')
        }
        roots['val'] = {
            'img': os.path.join(img_root, 'val2017'),
            'cap': os.path.join(ann_root, 'captions_val2017_tokenized.json')
        }
        roots['test'] = {
            'img': os.path.join(img_root, 'val2017'),
            'cap': os.path.join(ann_root, 'captions_val2017_tokenized.json')
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        for split in ['train', 'val', 'test']:
            ...
            if ids_dataset is None:
                ids = list(coco_dataset[0].getAnnIds())
            else:
